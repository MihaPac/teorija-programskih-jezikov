...
evalCmd (suc n) (just st , nats) FOR x := a TO b DO cmd DONE =
          if evalBExp st (a < b)
            then evalCmd n (just st , nats) (cmd ； FOR x := (a + ` 1) TO b DO cmd DONE)
          else (just st , nats) -- Se dejansko naj izpise *stevec*?? Verjetno ne
-- Ideja: Ce je PRINT pred neko komando, bo izvedel komando normalno in dodal stevila v nats!
